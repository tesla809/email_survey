...
router.get('/current_user', (req, res) => {  // If req.user exists, auth works.
  ...
  res.send(req.user); // send user. Make sure that you sign in with /auth/google first. If client side gets req.user, logged in. If not (undefined or null, etc.) not logged in
});
